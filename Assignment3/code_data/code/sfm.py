...
def main():
    ...
    # TODO Estimate relative pose of first pair --> DONE!
    ...
    # TODO --> DONE!
    e_im1.SetPose(np.eye(3), np.array([0, 0, 0]))
    longest = 0
    longest_idx = 0
    for i in range(len(possible_relative_poses)):
        e_im2.SetPose((possible_relative_poses[i][0]), possible_relative_poses[i][1])
        # TODO Triangulate initial points --> DONE!
        points3D_possible, _, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)
        if len(points3D_possible) > longest:
            longest = len(points3D_possible)
            longest_idx = i

    # Set the image poses in the images (image.SetPose(...))
    e_im2.SetPose((possible_relative_poses[longest_idx][0]), possible_relative_poses[longest_idx][1])
    ...
    while len(registered_images) < len(images):
        for image_name in images:
            ...
            # With too few correspondences the pose estimation becomes shaky.
    ...
    # If we want to not plot the 'outliers' / points from the remote wall, to concentrate on the fountain point cloud
    # points3D = points3D*np.expand_dims(np.abs(points3D[:, -1]) <= 6.0, 1)
